...
{ A Figura~\ref{fig:cpu_abstraction} é uma representação simplificada de um
    processador. A unidade de controle lê uma instrução da memória, a
    decodifica e comanda o processador; o circuito de lógica combinacional lê
    os dados dos registradores, entrada e memória conforme necessário, executa
    a instrução decodificada e escreve no banco de registradores, na memória de
    dados ou na saída se for preciso; a unidade de controle lê uma nova instrução
    e o ciclo se repete até o fim do programa. A posição de memória da instrução
...
{ O \textit{design} do módulo visa reduzir o \textit{hardware} necesário para
    uma implementação mínima, sendo capaz de emular por \textit{software} as
    extensões \textbf{M} (de \textit{\textbf{M}ultiplication and Division}),
    \textbf{F} (de \textit{Single-Precision \textbf{F}loating-Point}) e \textbf{D}
    (de \textit{\textbf{D}ouble-Precision Floating-Point}).
}
...
\end{figure}
    ...
    { Existem propostas de adição de algumas extensões na especificação da
        ...
        ratificadas. Há também a possibilidade de se desenvolver extensões
        próprias. A regra de nomenclatura para esse caso é iniciar o nome da
        extensão com um \textbf{X} seguido por um nome alfabético e.g.
        \textbf{Xhwacha}.
    }
    ...
    { Para a \textit{ISA RISC-V}, existem quatro níveis de privilégio de acesso,
        ...
        mais simples trata \textit{syscalls}), o de usuário (nível da
        aplicação do usuário), o de supervisor (sistema operacional)
        e o de hipervisor (virtualização).
    }
...
\begin{figure}[H]
\centering
    \includegraphics[width=.95\linewidth]{../images/rars2.png}
...
{ Processadores são circuitos elétricos, e podem ser descritos por diagramas.
    ...
    \begin{figure}[H]
    \centering
        \includegraphics[width=.5\linewidth]{../images/pipeline_4_stage.png}
